...
const criarCozinha = async (request, response) => {
    ...
    const buscaBairros = await CozinhaSchema.find({ bairro: bairro })
    //filtrei as cozinhas que tem o bairro que a pessoa digitou
    let BairrosExistentes = buscaBairros.filter((cozinha) => cozinha.endereco.bairro === bairro)
    //verifiquei se vai encontrar no array do filter UMA cozinha
    let nomeBairroExistente = BairrosExistentes.find((cozinha) => cozinha.nome === nome)
    if (nomeBairroExistente) {
